...
export default defineComponent({
  ...
  setup() {
        ...
        //trick to remove microsoft orgs informaion
        if(config.isMSFT){
          orgs.orgs = orgs.orgs.filter((org: GHOrg):boolean => {
            return org.login !== 'MicrosoftCopilot';
          });
        }
        ...
        console.log('orgs in config - after:', config.github.allGithubOrgs);
        console.log('teams in config - after', config.github.allGithubTeams);
    ...
    const fetchMetricsData = async () => {
      try {
        ...
        if (!entAndOrg) {
          ...
          console.log('metrics for ent&org:', metrics.value);
        }
